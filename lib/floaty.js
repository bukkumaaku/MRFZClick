    ...
    <card w="195" h="170" id="mainFloaty">
        <vertical bg="#60000000">
            <horizontal gravity="right" bg="#50000000">
                ...
                <img src="@drawable/ic_play_circle_outline_black_48dp" tint="#fa5858" w="25" h="25" id="StartClick" />
                <img src="@drawable/ic_remove_black_48dp" w="25" h="25" tint="white" id="minWindow" />
                <img src="@drawable/ic_highlight_off_black_48dp" w="25" h="25" tint="white" id="closeWindow" />
            </horizontal>
            <text id="settitle" h="25" text="未开始" textColor="#ffffff" gravity="center_vertical" bg="#900174df"/>
            <vertical w="*" h="120">
                <ScrollView id="sb" h="*" scrollbars="vertical" fadingEdge="vertical">
            ...
            if (Math.abs(event.getRawY() - y) < 5 && Math.abs(event.getRawX() - x) < 5) {
                ...
                ui.run(function() {
                    if (win.mainFloaty.attr("h") != "25") {
                        ...
                        win.minWindow.attr("src", "@drawable/ic_zoom_out_map_black_48dp");
                    } else {
                        win.mainFloaty.attr("h", winHeight);
                        win.mainFloaty.attr("w", winWidth);
                        win.closeWindow.setVisibility(View.VISIBLE);
                        win.setPosition(wx - (bw / 25) * 140, wy);
                        win.minWindow.attr("src", "@drawable/ic_remove_black_48dp");
                    }
                });
            }
            break;
...
win.StartClick.click(function() {
    ui.run(function() {
        if (win.StartClick.attr("tint") == "#fa5858") {
            ...
            win.StartClick.attr("src", "@drawable/ic_pause_circle_outline_black_48dp");
            ...
            win.StartClick.attr("src", "@drawable/ic_play_circle_outline_black_48dp");
...
events.on("sendmessage", function(words) {
    ui.run(function() {
        win.LogTxt.setText(win.LogTxt.getText() + (win.LogTxt.getText() ? "\n" : "") + words);
