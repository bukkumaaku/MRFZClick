...
function startClick() {
	...
	while (Action) {
		...
		if (result || result2) {
			toastLog("点击 蓝·开始行动"); fatherengine.emit("settitle", "第" + CountBattleNumber + "轮行动");
		...
		if (result || result1) {
			...
			SleepTime = 5000;
			if ((setting.isRecycle && CountBattleNumber > Number(setting.recycles)) || CountMaterial(capture)) {
		...
		if (result && result2 && setting.UseOriginiums && UseOriginiumsCount < Number(setting.UseOriginiumsNumber)) {
			...
			(result && result2 && ((setting.UseOriginiums && UseOriginiumsCount >= Number(setting.UseOriginiumsNumber)) || !setting.UseOriginiums)) ||
			(result && !result2)
		) {
...
function CountMaterial(capture) {
	...
	for (let i = 0; i < MaterialList.name.length; i++) {
		...
		SendM(MaterialList.name[i] + ":目标" + MaterialList.number[i] + " 已得" + MaterialList.done[i]);
		if (MaterialList.number[i] - MaterialList.done[i] >= 1) complete = false;
	}
	MRFZ.RecycleCapture();
