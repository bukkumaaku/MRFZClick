var setting = JSON.parse(files.read("./setting.js", (encoding = "utf-8"))); //读取配置文件
var MRFZ = require("./lib/MRFZ.js"); //读取识图函数库
var storage = storages.create("MRFZClickMaterialList"); //获取是要识别的材料列表
var MaterialList = JSON.parse(storage.get("list")); //获取全部材料列表
const _sleep = sleep;
...
var SendM = (e) => {
	log(e);
	fatherengine.emit("sendmessage", e); //给悬浮窗发送信息
};
toastLog = (e) => {
	//重写toastLog
	_toastLog(e);
...
function CountMaterial(capture) {
	...
	MRFZ.RecycleCapture(); //销毁图片
	storage.put("list", MaterialList);
	return complete;
}

function ScaleSet(SplitScreen) {
	//判断游戏缩放比例
...
function OrientationChanged() {
	...
	this.listener = JavaAdapter(
		android.view.OrientationListener,
		{
			onOrientationChanged: function () {
				MRFZ.ScaleNum = ScaleSet(this.currentPosition());
			},
		},
		context
	);
